...
export function expandIn(el: HTMLElement, duration = 0.25): gsap.core.Tween | undefined {
  if (prefersReducedMotion()) return undefined
  return gsap.fromTo(el, { opacity: 0, height: 0 }, { opacity: 1, height: 'auto', duration, ease: 'power2.out' })
}

export function collapseOut(el: HTMLElement, duration = 0.2): gsap.core.Tween | undefined {
  if (prefersReducedMotion()) return undefined
...
export function staggerFadeIn(elements: HTMLElement[], duration = 0.3, stagger = 0.06) {
  if (prefersReducedMotion()) return
  gsap.fromTo(elements, { opacity: 0, y: 12 }, { opacity: 1, y: 0, duration, stagger, ease: 'power2.out' })
}
