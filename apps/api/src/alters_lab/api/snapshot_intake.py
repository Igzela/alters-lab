...
def get_snapshot_persist_target_path() -> Path:
    repo_root = Path(__file__).resolve().parents[5]
    return repo_root / "alters/current/snapshot.yaml"


def get_snapshot_persist_audit_log_path() -> Path:
    repo_root = Path(__file__).resolve().parents[5]
    return repo_root / "docs/harness/phase3_write_audit.jsonl"


def get_snapshot_persist_backup_dir() -> Path:
    repo_root = Path(__file__).resolve().parents[5]
...
def persist_snapshot(session_id: UUID, body: SnapshotPersistRequest):
    ...
    target = get_snapshot_persist_target_path()
    audit_log = get_snapshot_persist_audit_log_path()
    backup = get_snapshot_persist_backup_dir()

    if body.dry_run:
        result = preview_snapshot_persist(session.snapshot, target)
        boundary = {
    ...
    result = write_snapshot_with_audit(
        session.snapshot,
        target,
        audit_log,
        body.approval_token,
