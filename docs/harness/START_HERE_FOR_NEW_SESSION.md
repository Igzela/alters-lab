...
- **P11**: Product Completeness Before Validation. P11-000 done. P11-M1 done (inventory). P11-M1-R1/R2/R3 done (granularity refinement, count fix, wording fix). P11-M2 done (workflow gap map: 23 workflows, 4 tiers, 20 gaps, 0 normal-use blockers, 4 P6 blockers). P11-M3 done (UX gaps: 12 gaps, 0 blockers, 3 medium, 9 low). P11-M4 done (gap closure plan: all frontend-only, existing APIs sufficient). P11-M5 next (implementation batch 1).
